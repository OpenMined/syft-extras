...
def handle_message(data):
    ...
    # Always send the message to the recipient (even if it's yourself)
    if recipient in user_sessions:
        ...
        # Wait for human response, schedule AI backup
        pending_messages[recipient].append(message_id)
        schedule_ai_response(sender, recipient, message, message_id)
    else:
        # Recipient is offline, get immediate AI response
        conversation_id = f"{sender}_{recipient}"
        response_text, resp_message_id = get_ai_response(message, conversation_id)
        ...
        message_store[resp_message_id] = {
            "response": response_text,
            "timestamp": timestamp,
            "sender": recipient,  # AI responding on behalf of recipient
            "recipient": sender,
            "original_message": message,
            "ai_generated": True
        }
