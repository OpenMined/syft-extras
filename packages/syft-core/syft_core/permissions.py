...
from .constants import PERM_FILE
from .types import AbsolutePath, PathLike, RelativePath
...
class SyftPermission(BaseModel):
    ...
    @classmethod
    def create(cls, context: Client, dir: Path) -> "SyftPermission":
    ...
    @classmethod
    def datasite_default(cls, context: Client, dir: Path) -> "SyftPermission":
    ...
    @classmethod
    def mine_with_public_read(cls, context: Client, dir: Path) -> "SyftPermission":
        perm = cls.create(context, dir)
    ...
    @classmethod
    def mine_with_public_write(cls, context: Client, dir: Path) -> "SyftPermission":
        # for backwards compatibility
        return cls.mine_with_public_rw(context, dir)

    @classmethod
    def mine_with_public_rw(cls, context: Client, dir: Path) -> "SyftPermission":
        perm = cls.create(context, dir)
        perm.add_rule(path="**", user=context.email, permission=["admin"])
...
class ComputedPermission(BaseModel):
    ...
    def from_user_rules_and_path(
        cls, rules: List[PermissionRule], user: str, path: RelativePath
    ) -> "ComputedPermission":
...
def convert_permission(old_perm_dict: dict) -> list:
    old_perm_dict.pop(
        "filepath", None
    )  # not needed, we use the actual path of the perm file
    ...
    for email in user_permissions:
        new_perm_dict = {
            ...
            "path": "**",
            "user": (
                email if email != "GLOBAL" else "*"
            ),  # "*" is a wildcard for all users
        }
...
def migrate_permissions(snapshot_folder: Path) -> None:
    ...
    for file in files:
        ...
        try:
            ...
            continue
        new_file_path = file.with_name(
            file.name.replace(old_syftperm_filename, PERM_FILE)
        )
        logger.info(
