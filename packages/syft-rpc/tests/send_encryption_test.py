...
def test_send_caching_with_encryption(alice_client: Client, bob_client: Client):
    """Test that caching behavior with encrypted messages.

    Note: Encrypted messages don't cache effectively because each encryption
    uses different ephemeral keys, resulting in different message IDs.
    This test validates that behavior.
    """
    url = make_url(bob_client.email, "test_app", "endpoint")
    body = {"message": "cached"}

    # First call: creates new cached request
    future1 = send(url=url, body=body, encrypt=True, cache=True, client=alice_client)

    # Verify the request file was created
    request_file1 = future1.path / f"{future1.id}.request"
    assert request_file1.exists()

    # Second call with same parameters: creates NEW request due to different encryption
    future2 = send(url=url, body=body, encrypt=True, cache=True, client=alice_client)

    # With encryption, each call generates different ephemeral keys
    # So IDs will be different (no effective caching)
    assert future2.id != future1.id  # Different IDs due to different encrypted bodies
    assert future2.path == future1.path  # Same path though

    # Both should have different encrypted bodies (different ephemeral keys)
    assert future2.request.body != future1.request.body

    # Both files should exist
    request_file2 = future2.path / f"{future2.id}.request"
    assert request_file2.exists()
    assert request_file1.exists()

    # Both should be valid encrypted payloads with same sender/receiver
    payload1 = EncryptedPayload.model_validate_json(future1.request.body.decode())
    payload2 = EncryptedPayload.model_validate_json(future2.request.body.decode())

    assert payload1.sender == payload2.sender == alice_client.email
    assert payload1.receiver == payload2.receiver == bob_client.email
